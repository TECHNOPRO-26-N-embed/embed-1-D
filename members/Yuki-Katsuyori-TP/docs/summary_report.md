# 成果発表レポート

> 記入者: （名前）
> グループ: （グループID）
> 日付: 2026/05/27

> **📝 このレポートをそのまま発表原稿にできます。**
> 各セクションの指示を消して、自分の言葉で書いてください。
> 1人 **約2分**（グループ5人で10〜12分）に収まる分量が目安です。

---

## 1. 何を作ったか（30秒）

<!-- ひと言で伝わるように書いてください。「○○を使って、△△するガジェットを作りました。」 -->

**ガジェット名：**　距離警告ブザーシステム

**ひと言で説明：**　超音波センサーで物体との距離を測り、近づくほど警告音が変化する安全ブザーを作りました。

**使った部品：**
- Arduino Uno
- 超音波距離センサー（HC-SR04）
- パッシブブザー
- ジャンパー線

---

## 2. 設計で考えたこと（15秒）

<!-- 要件定義・基本設計・詳細設計の中で、自分なりに考えたこと・工夫した点を書いてください -->
<!-- 「なぜこの部品を選んだか」「なぜこの構成にしたか」など、判断の理由を言葉にしてください -->

・距離によって危険度が変わるので、音の高さと鳴り方を4段階に分ける設計にしました。

・誤動作を防ぐために、ヒステリシス（50cm/55cm）を入れて音がバタつかないようにしました。

・HIGH の音は連続音ではなく、高速ビープにして緊急性を高める工夫をしました。


---

## 3. できたこと・できなかったこと（30秒）

<!-- 正直に書いてください。「できなかった」も立派な成果です -->

**動いたもの：**
- 距離に応じて HIGH / MID / LOW / SILENT の4段階で音が変化

- HIGH の高速ビープ（50ms周期）が正しく動作

- センサー値が安定し、誤検知がほぼない



**動かなかった・間に合わなかったもの：**
- LED の4段階警告
- なぜ動かなかったか（わかる範囲で）： 時間の都合で実装まで進められなかったため

---

## 4. 一番苦労したこと、どう乗り越えたか（30秒）

<!-- ここが発表の山場です。1つだけに絞って、ストーリーで書いてください -->

**何が起きたか：**　距離が近いときに音がバタバタ切り替わり、不安定な警告音になってしまった。


**どう対処したか：**　原因を調べた結果、距離の揺れによる誤判定だとわかったので、ヒステリシス（50cm/55cm）を導入して状態が安定するようにした。


**そこから何がわかったか：**　センサーは思ったよりブレるので、ソフト側で落ち着かせる工夫が必要だと気づいた。


---

## 5. 学んだこと・今後の展望（25秒）

<!-- 「勉強になった」ではなく具体的に。 -->
<!-- 例: 「AIが生成したコードをそのまま使ったら動かず、自分でprintfデバッグして原因を特定した」 -->
<!-- 例: 「配線図を書かずに組んだら混乱した。図を書いてからやり直したらすぐ動いた」 -->

**学んだこと：**
- センサー値は常に揺れるため、異常値処理やヒステリシスが必須

**今後の展望（この仕組みを発展させるなら）：**
<!-- 例: 「温度センサー＋モーターの組み合わせを応用すれば、室温に応じて自動で換気する仕組みが作れそう」 -->

- LED を追加して、視覚的にも危険度がわかるようにしたい

---

## 6. 発表で見せたいもの（メモ）

<!-- 動くデモ、配線の写真、設計図、フローチャート、AIとのやり取りのスクショなど -->
<!-- 動くものがなくても、「考えた過程」を見せられれば十分です -->

- 

---

> **💡 書き終わったら**
> - 声に出して読んで、2分に収まるか確認してください
> - 長すぎたら「4. 苦労したこと」を1つに絞りましょう
> - グループで導入・締めをつけたい場合は、次の時間に相談して決めてください
